...
export class BufferingEventEmitter {
    ...
    private popUnhandledMessage(type: string) {
        ...
        if (index >= 0) {
            this.logger.info("Releasing buffered message of type '" + type + "'");
    ...
    emit(eventType: string, ...args: any) {
        if (this.eventEmitter.listenerCount(eventType) === 0) {
            this.logger.warn("No listener registered for '" + eventType + "', buffering");
    ...
    private bufferEvent(eventType: string, args: any[]) {
        if (this.bufferedEvents.length === this.maxBufferSize) {
            ...
            this.logger.warn(`Buffered event handler overflowing, expiring '${expiredEvent.type}' event`);
        }
